...
export const ProjectCard = (props: { project?: Project, showMenu?: boolean, showAdd?: boolean }) => {
  ...
  function subProjectNameTrans(value: string) {
    ...
    if (count > 1) {
      return <>
        <div className={styles.subPath}>{value.substring(0, value.indexOf('/'))}</div>
        /
        <i className="al-ico-dots al-icon" />
        <div className={styles.subPath}>{value.substring(value.lastIndexOf('/'))}</div>
      </>;
    }
    return <div className={classNames(styles.subPath, styles.doubleWidth)}>{value}</div>;
  }
    ...
    <div className={styles.projectCard}>
      <div className={styles.folderTab}>
        <div className={styles.firstTab} onClick={() => setShowSec(false)}>
          {!showSec ? (
            <svg
              xmlns="http://www.w3.org/2000/svg" width="64" height="20" viewBox="0 0 64 20"
            >
              {project?.sub_projects?.length && secHovered && (
                <path
                  d="M30.06,0H64V20H30.06C29.93,19.78,30.06,11.93,30.06,0Z" fill="#5a658e"
                />
              )}
              {project?.sub_projects?.length && !secHovered && (
                <path
                  d="M30.06,0H64V20H30.06C29.93,19.78,30.06,11.93,30.06,0Z" fill="#2c3246"
                />
              )}
              {mainHovered ? (
                <path
                  d="M35.61,0H6A6,6,0,0,0,0,6V20H60C44.82,12.36,49.37,0,35.61,0Z" fill="#5a658e"
                  onMouseEnter={() => setMainHovered(true)}
                  onMouseLeave={() => setMainHovered(false)}
                />
              ) : (
                <path
                  d="M35.61,0H6A6,6,0,0,0,0,6V20H60C44.82,12.36,49.37,0,35.61,0Z" fill="#384161"
                  onMouseEnter={() => setMainHovered(true)}
                  onMouseLeave={() => setMainHovered(false)}
                />
              )}
            ...
            >
              ...
              <path
                d="M64,0V20H30C45.18,12.36,40.63,0,54.39,0Z" fill="#2c3246"
              />
            </svg>
          )}
        </div>
      ...
      <div className={styles.cardContainer}>
          ...
          >
            <header>
              <div className={styles.cardName}>
                <Tooltip title={project.name} placement="bottom" color={"blue"}>
                  <span className={styles.projectName}>{shortProjectName(project.name)}</span>
                </Tooltip>
                {showRename && (
                  <Space>
                    <Input />
            ...
            <div className={styles.cardBody}>
              <div className={styles.cardCounter}>
                <CircleCounter counter={project.stats?.active?.total_tasks ?? "0"} label={"TOTAL"}
                               type={project.stats?.active?.total_tasks === 0 ? CircleTypeEnum.empty : CircleTypeEnum.pending} />
                <CircleCounter counter={project.stats?.active?.status_count?.in_progress ?? "0"} label={"RUNNING"}
                               type={project.stats?.active?.status_count?.in_progress === 0 ? CircleTypeEnum.empty : CircleTypeEnum.running} />
                <CircleCounter counter={project.stats?.active?.completed_tasks_24h ?? "0"}
                               label={"COMPLETED"}
                               underLabel="(24 hrs)"
            ...
            <footer>
              {project?.stats && <div className={styles.footerTitle}>
                COMPUTE TIME: {convertSecToDaysHrsMinsSec(project.stats?.active?.total_runtime ?? 0)}
              </div>}
            </footer>
          </div>
        ) : (
        ...
        {showAdd &&
          <div
            ...
            onMouseEnter={() => setMainHovered(true)}
            onMouseLeave={() => setMainHovered(false)}>
            <div className={classNames("al-icon al-ico-plus al-color", styles.plusIcon)}></div>
          </div>
        }
      </div>
    </div>
